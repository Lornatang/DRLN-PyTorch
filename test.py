...
def main() -> None:
    ...
    model = DRLN(config.upscale_factor).to(device=config.device, memory_format=torch.channels_last)
    print("Build DRLN model successfully.")

    # Load the super-resolution model weights
    checkpoint = torch.load(config.model_path, map_location=lambda storage, loc: storage)
    model.load_state_dict(checkpoint["state_dict"])
    print(f"Load DRLN model weights `{os.path.abspath(config.model_path)}` successfully.")
